...
<p>
<a class="zoomable" href={DirectoryStructure}><img decoding="async" loading="lazy" alt="Directory structure for my main project folder" class="img_ev3q" src={DirectoryStructure}/></a>
...
I'd recommend a project like this to anyone who is comfortable with Git and GitHub, who feels like exploring Pages and maybe dipping a toe into Actions. 
...
The next few sections will give some details and starting points for some of these setup steps. If that's not for you, skip to my [plans for the future](#plans-for-the-future)!

### GitHub + Pages

[GitHub Pages](https://pages.github.com/) is GitHub's free hosting service for [static sites](https://en.wikipedia.org/wiki/Static_web_page). If you have a GitHub account, you can put up a basic website pretty quickly. Usually, you'll use some static site framework or even something like [webpack](https://webpack.js.org/) to convert your dev-friendly source files into a bundle that GitHub Pages is already preconfigured to deploy as a live site. GitHub wants you to use Pages ["to host a website about yourself, your organization, or your project"](https://docs.github.com/en/pages/getting-started-with-github-pages/about-github-pages). Generally-speaking, don't plan to host your money-making businessy websites for free on Pages.

This is a "yourself" kinda website, so its GitHub repository uses the special name set aside for my User Site: [IAmKelDev/iamkeldev.github.io](https://github.com/IAmKelDev/iamkeldev.github.io). With some configuration tweaks you can tell GitHub to use your custom domain instead of the auto-assigned domain you get for free. My free domain was https://iamkeldev.github.io, which now just redirects to https://keldev.net.
...
This step has the largest time-sink potential, but *the bare minimum is still quick!* Remember, your personal site doesn't have to be perfect immediately. Tinker as you go! I'm still making style adjustments as I write this post.

You get a fairly complete site skeleton just from installing the packed-in example site. If you want to poke around without putting any files on your computer, use one of the in-browser sandboxes recommended on the [Docusarus Playground page](https://docusaurus.io/docs/playground). Otherwise, the main [Installation Guide](https://docusaurus.io/docs/installation) goes through the basics in just a couple minutes. In the simplest case, you might decide to just throw out the default Markdown content files in the example site, replace them with your own, and call it done.

If you need something different, you can look at configuring different folders as instances of one of the main Docusaurus [Content Plugins](https://docusaurus.io/docs/api/plugins#content-plugins) to create new [Docs](https://docusaurus.io/docs/docs-introduction), [Pages](https://docusaurus.io/docs/creating-pages), or [Blog](https://docusaurus.io/docs/blog) sections for your site.

Much of your overall site layout and technical setup comes from the main `docusaurus.config.(ts/js)` file. There's a lot of config stuff you can set. [Here's my (fairly standard) config](https://github.com/IAmKelDev/iamkeldev.github.io/blob/7d664d31c2dd92cfe6e312963c869f9116004d08/docusaurus.config.ts) as of this post. Whenever you're unsure what config options are even available for your plugins, or what markdown frontmatter you can use, refer to the plugin API pages, which are all in [this plugins list](https://docusaurus.io/docs/api/plugins).

#### Going further
Once you've got your basic configuration and layout stuff done, you should be more than ready to start putting up content.  
When you want to go further:
- There are [community plugins](https://docusaurus.community/plugindirectory/) out there.
- For blogging specifically, you may find it useful to [define global author info](https://docusaurus.io/docs/blog#global-authors) (or remove all the example authors), and to set up [post tagging](https://docusaurus.io/docs/blog#blog-post-tags)

Possibly the biggest time sink here is all that you can do by modifying site-wide styling in `src/css/custom.css`. Be sure to look at the [Styling and Layout](https://docusaurus.io/docs/styling-layout) guide. Not only are there a lot of global Infima variables for things like color, but you can also use this space to define style rules for any HTML that you embed in your markdown content.

### Automatic deployment with GitHub Actions

Maybe put my GH action in here as a code block? And explain it? It's pretty quick.
