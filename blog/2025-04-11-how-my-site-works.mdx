...
- **A personal site**, with some about/bio information, a space for descriptions of my projects, and a blog with a feed of posts (with [RSS](https://en.wikipedia.org/wiki/RSS)).
...
Their recent [Bear Manifesto](https://herman.bearblog.dev/manifesto/) addresses a lot of my points of irritation. If you have markdown content that you want to turn into a personal website somewhere, and you're not too concerned about styling, you can easily throw it in there. It's lightweight by design, but I found I wanted more style control and extensibility.

### Landing on something

My job is what finally pushed me over that initial learning curve for a static site framework. We needed a reasonably polished site, using open source or free tools wherever possible. That's how [the OpenDI website](https://opendi.org/) (and my own personal site in turn) wound up using [Docusaurus](https://docusaurus.io/). I stuck pretty close to the default Docusaurus styling with the OpenDI site.
...
Meanwhile, I've got plenty of room for expansion and customization. I've already done some basic style tweaks. If you're familiar with the hallmarks of a Docusaurus website layout then mine will look familiar, but I'm hoping the cookie-cutter effect won't be too strong for most readers.
...
With a preview instance running, Docusaurus scans for file changes and regenerates the preview site in realtime wenever I save. This is a prime intended workflow, so you can see your content in its final form as you edit. Local preview functionality comes with the default project setup. It's a single command to spin up each time.
...
A common quick fix is to just throw your images on an existing image hosting provider, but some of my [toolset irritations](#picking-a-toolset) apply there.

I [already use Backblaze B2](/about/software-I-use/#backblaze-b2) for general cloud storage. You can make your Backblaze Buckets public, and a little research turned up some [official Backblaze docs](https://www.backblaze.com/docs/cloud-storage-deliver-public-backblaze-b2-content-through-cloudflare-cdn) describing how to use a public B2 bucket for media delivery via Cloudflare CDN. I don't mind the Backblaze rates for storage, so if it's possible to set up the Cloudflare end of this on a free plan there, this could be a perfect solution for me.

### Image gallery

I take pictures for fun sometimes! I mostly just use my phone. Sometimes it bothers me to think about how many photos I have enjoyed taking, maybe shared with one or two people, and then left to sit for eternity. My local storage got so backed up on my phone that I finally caved and paid for some more Google storage a while back. But _I [already use Backblaze B2](/about/software-I-use/#backblaze-b2) for general cloud storage!_ I have needed to move my phone pictures over to Backblaze for too long.
...
I go back and forth on analytics. I don't want detailed click heatmaps or realtime session recordings ([this is too much](https://clarity.microsoft.com/)). I just want to see if any of my blog posts get unexpected traffic. I'll probably look at setting up [GoatCounter](https://www.goatcounter.com/) with [this community plugin](https://github.com/Addono/docusaurus-plugin-goatcounter).

### Email list

The built-in RSS feed in the Docusaurus Blog plugin is great! More people use email than RSS though, so it's a lower barrier subscription option that I'd like to include. I imagine this will be a better than the [image gallery idea](#image-gallery) as a first project for extending the basic Docusaurus functionality, and [this walkthrough](https://heromodeapp.com/blog/docusaurus-blog-customizations) looks promising, so I'll be giving that a try soon!

## Thanks for reading!

This actually took _forever_ to write. If you have thoughts or questions... Email me?  
